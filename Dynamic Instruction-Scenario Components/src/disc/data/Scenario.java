 ...
 * {@link Instruction}s with attached arguments such as the name and additional information
 * for control flow (eg start position for the given scenario).
 * <p>
 * Name and other arguments are looked for in any line starting with two '#' characters.
 * <p>
 * eg:<br>
 ...
 * <p>
 * The Scenario constructor does not care what the name of the argument is, unless it is specifically
 * "name", and it will place the arguments into the "args" array in order of finding it in the file.
 * <p>
 * Comments are begun with a single '#' character,  and ignored by the constructor.
 * <p>
 * eg:<br>
 * #This is a comment.
 * 
...
public class Scenario {
	 ...
	 * Calls the {@link Instruction} constructor for each non-comment, non-empty line.
	 * 
	 * @param scenarioFile A File object of a Scenario file.
	...
	public Scenario(File scenarioFile) throws FileNotFoundException {
		Scanner scn = new Scanner(scenarioFile);
		...
		while(scn.hasNextLine()) {
			...
			} else if(!((tmp.startsWith("#") || tmp.startsWith("# ")) || tmp.isEmpty())) toInstructions.add(new Instruction(tmp));
		} scn.close();
	...
	public Scenario(String scenarioFile) {
		...
		while(scn.hasNextLine()) {
			String tmp = scn.nextLine().trim();
			if(tmp.startsWith("##")) {
				if(tmp.contains("name=")) scenarioName = tmp.substring(7);
				else toArgs.add(tmp.substring(tmp.indexOf('=') + 1));
			} else if(!tmp.contains("#") && !tmp.isEmpty()) toInstructions.add(new Instruction(tmp));
		} scn.close();
		args = toArgs.toArray(new String[toArgs.size()]);
		instructions = toInstructions.toArray(new Instruction[toInstructions.size()]);
	}
	...
	public Queue<Instruction> getInstructionQueue() {
		...
		for(Instruction i : instructions) q.add(i);
		return q;
	}
	...
	/**
	 * @return a String that fully represents the scenario, as would be found in a Scenario file.
	 */
	@Override
	public String toString() {
		StringBuilder sb = new StringBuilder("");
		sb.append("##name=" + scenarioName + "\r\n");
		for(int i = 0; i < args.length; i++) sb.append("##arg" + i + "=" + args[i] + "\r\n");
		sb.append("\r\n");
		for(int i = 0; i < instructions.length; i++) if(!instructions[i].toString().equals("NaN")) sb.append(instructions[i].toString()+ "\r\n");
		return sb.toString();
	}
	
	/**
	 * Calculates a hash code using the hashCode() methods of the "scenarioName", "args", and each
	 * of the {@link Instruction}s stored in the {@link Scenario}.
	 * 
